...
public class GetExperts {
    public static ArrayList<Object> getExpertsFE(
        ...
        int object_item_1 = jdbcTemplate.queryForList("select expert_value from xml_project.public.experts_object_xml where id_file=? and name_company=?", IdFile.toString(), NameCompany).size();
        for (int i = 1; i < object_item_1; i++) {
            // expert_value
            arrayList1.put("name_" + i + "_expert_value", jdbcTemplate.queryForList("select * from xml_project.public.experts_object_xml where id_file=? and name_company=?", IdFile.toString(), NameCompany).get(i).get("expert_value").toString());
            // family_name_value
            arrayList1.put("name_" + i + "_family_name_value", jdbcTemplate.queryForList("select * from xml_project.public.experts_object_xml where id_file=? and name_company=?", IdFile.toString(), NameCompany).get(i).get("family_name_value").toString());
            // first_name_value
            arrayList1.put("name_" + i + "_first_name_value", jdbcTemplate.queryForList("select * from xml_project.public.experts_object_xml where id_file=? and name_company=?", IdFile.toString(), NameCompany).get(i).get("first_name_value").toString());
            // second_name_value
            arrayList1.put("name_" + i + "_second_name_value", jdbcTemplate.queryForList("select * from xml_project.public.experts_object_xml where id_file=? and name_company=?", IdFile.toString(), NameCompany).get(i).get("second_name_value").toString());
            // expert_type_value
            arrayList1.put("name_" + i + "_expert_type_value", jdbcTemplate.queryForList("select * from xml_project.public.experts_object_xml where id_file=? and name_company=?", IdFile.toString(), NameCompany).get(i).get("expert_type_value").toString());
            // expert_certificate_value
            arrayList1.put("name_" + i + "_expert_certificate_value", jdbcTemplate.queryForList("select * from xml_project.public.experts_object_xml where id_file=? and name_company=?", IdFile.toString(), NameCompany).get(i).get("expert_certificate_value").toString());
            // expert_certificate_begindate_value
            arrayList1.put("name_" + i + "_expert_certificate_begindate_value", jdbcTemplate.queryForList("select * from xml_project.public.experts_object_xml where id_file=? and name_company=?", IdFile.toString(), NameCompany).get(i).get("expert_certificate_begindate_value").toString());
            // expert_certificate_end_date_value
            arrayList1.put("name_" + i + "_expert_certificate_end_date_value", jdbcTemplate.queryForList("select * from xml_project.public.experts_object_xml where id_file=? and name_company=?", IdFile.toString(), NameCompany).get(i).get("expert_certificate_end_date_value").toString());
            // id_transaction
        ...
        objectHashMap.put("count_object_items", jdbcTemplate.queryForList("select expert_value from xml_project.public.experts_object_xml where id_file=? and name_company=?", IdFile.toString(), NameCompany).size());
    ...
    public static Node getExperts(
        ...
        if (!file.exists()) {
            ...
            jdbcTemplate.update("insert into " +
                            ...
                            "?, ?, ?, ?, ?, ?)",
                    "Данных нет", "Матвеев", "Алексей", "Александрович",
                    "Нет данных", "МС-Э-1-10-13222", "2020-01-29", "2025-01-29",
